...
export default function Playground() {
  ...
  return (
    <section id="playground" className="py-14 sm:py-20 lg:py-24 bg-canvas-subtle/60 border-y-2 border-ink relative overflow-hidden scroll-mt-20 sm:scroll-mt-28">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 space-y-8 sm:space-y-10">
        ...
        <div className="flex flex-wrap items-center justify-center gap-2 sm:gap-3">
          <motion.button
            ...
            className={`px-3.5 sm:px-5 py-2 sm:py-2.5 rounded-xl font-display font-bold text-[11px] sm:text-xs uppercase tracking-wider border-2 transition-all duration-150 ${
              activeTab === 'type'
        ...
        <AnimatePresence mode="wait">
          ...
          {activeTab === 'type' && (
            <motion.div
              ...
              className="p-5 sm:p-8 md:p-12 rounded-2xl sm:rounded-3xl border-2 border-ink bg-white shadow-tactile-lg space-y-6 sm:space-y-8"
            >
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-center">
                ...
                <div className="lg:col-span-5 space-y-4 sm:space-y-6">
                  <div className="space-y-1">
                    ...
                    <h3 className="font-display font-black text-xl sm:text-2xl text-ink uppercase">
                  ...
                  <div className="space-y-3 sm:space-y-4">
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full p-2.5 sm:p-3 rounded-xl border-2 border-ink font-display font-bold text-xs sm:text-sm bg-canvas focus:outline-none focus:ring-2 focus:ring-brand-coral"
                      />
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full accent-brand-coral h-2 bg-zinc-200 rounded-lg cursor-pointer"
                      />
                    </div>
                    ...
                    <div className="space-y-1.5">
                      ...
                      <input
                        ...
                        className="w-full accent-brand-cobalt h-2 bg-zinc-200 rounded-lg cursor-pointer"
                      />
                    </div>
                  </div>
                </div>
                ...
                {/* Live Preview Canvas */}
                <div className="lg:col-span-7 p-5 sm:p-8 rounded-2xl bg-canvas border-2 border-ink shadow-tactile min-h-[220px] sm:min-h-[260px] flex items-center justify-center overflow-hidden text-center">
                  <div
                    className="font-display font-black text-ink uppercase tracking-tight transition-all duration-100 break-words max-w-full overflow-hidden"
                    style={{
                      fontSize: `clamp(18px, ${fontSize}px, 64px)`,
            ...
            >
              <div className="grid grid-cols-1 lg:grid-cols-12 gap-6 sm:gap-8 items-center">
                      ...
                      <motion.button
                        ...
                        className={`p-2.5 sm:p-3 rounded-xl border-2 text-left transition-all ${
                          selectedPalette === idx
                            ...
                            : 'border-ink/10 hover:border-ink/40 bg-white'
                        }`}
                      >
                        <div className="text-xs font-display font-bold text-ink">{pal.name}</div>
                        <div className="flex gap-1.5 mt-2">
                          <span className={`w-3.5 sm:w-4 h-3.5 sm:h-4 rounded-full border border-ink ${pal.card}`} />
                          <span className={`w-3.5 sm:w-4 h-3.5 sm:h-4 rounded-full border border-ink ${pal.badge.split(' ')[0]}`} />
                        </div>
                      </motion.button>
                    ))}
                  </div>
                ...
                <div className="lg:col-span-7">
                  <div className={`p-5 sm:p-8 rounded-2xl border-2 border-ink shadow-tactile transition-colors duration-300 ${palettes[selectedPalette].bg}`}>
                    <div className={`p-4 sm:p-6 rounded-2xl border-2 border-ink shadow-tactile transition-colors duration-300 ${palettes[selectedPalette].card} ${palettes[selectedPalette].text} space-y-3 sm:space-y-4`}>
                      <div className="flex justify-between items-center gap-2">
                        <span className={`px-2.5 py-0.5 rounded-md text-[9px] sm:text-[10px] font-mono font-bold border border-ink shrink-0 ${palettes[selectedPalette].badge}`}>
                          SPECIAL OFFER
                        </span>
                        <span className="text-[10px] sm:text-xs font-mono">RIYA DESIGN LAB</span>
                      </div>

                      <h4 className="font-display font-black text-2xl sm:text-3xl lg:text-4xl leading-tight uppercase">
                  ...
                  <div className="flex flex-col gap-2 sm:gap-2.5">
                    ...
                    >
                      <span className="font-display font-bold text-xs uppercase flex items-center gap-2">
                        ...
                        <span>9:16 Vertical Story / Reel</span>
                      </span>
                      <span className="text-[10px] font-mono font-bold shrink-0">1080x1920</span>
                    </button>
                    ...
                    >
                      <span className="font-display font-bold text-xs uppercase flex items-center gap-2">
                        ...
                        <span>16:9 Web &amp; Banner Frame</span>
                      </span>
                      <span className="text-[10px] font-mono font-bold shrink-0">1920x1080</span>
                    </button>
                  </div>
                </div>

                {/* Aspect Ratio Canvas */}
                <div className="lg:col-span-7 flex items-center justify-center p-4 sm:p-6 bg-canvas rounded-2xl border-2 border-ink min-h-[300px] sm:min-h-[360px] overflow-hidden">
                  <div
                    className={`relative rounded-xl overflow-hidden border-2 border-ink shadow-tactile bg-white transition-all duration-300 ${
                      selectedFormat === 'story'
                        ? 'w-[180px] sm:w-[200px] aspect-[9/16]'
                        : selectedFormat === 'post'
                        ? 'w-[240px] sm:w-[280px] aspect-square'
